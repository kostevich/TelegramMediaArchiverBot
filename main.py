...
def bot_message(message):
    ...
    number_images = len(fnmatch.filter(os.listdir('upload/'),'*.jpg'))
    # Количество файлов с расширением .mp4 в папке.
    number_videos = len(fnmatch.filter(os.listdir('upload/'),'*.mp4'))
    # Вывод информации об успешности операции и количестве добавленных фотографий и видеофайлов.
    bot.send_message(message.chat.id, f'Добавлено {images} изображений и {videos} видеофайлов.')
...
def bot_message(message):
    ...
    if message.text == '📦 Начать сбор коллекции':
        ...
        bot.send_message(message.chat.id, f'Формирование коллекции окончано. Вы коллекции находится {number_files} файлов. Из них {number_images} изображений и {number_videos} видеофайлов..', reply_markup=markup)
        # Сохранение коллекции в архив.
        shutil.make_archive('archive', 'zip', 'upload' )
